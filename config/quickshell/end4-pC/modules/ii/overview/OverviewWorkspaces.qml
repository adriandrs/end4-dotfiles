...
Item {
    ...
    property real scale: Config.options.overview.scale * root.viewScale
    // Ajusta esto para igualar el tamano de la vista de ventanas.
    property real viewScale: 1.4
    ...
    function selectStep(delta) {
        if (root.cellCount === 0) return;
        if (root.selectedIndex < 0) {
            root.selectedIndex = 0;
            return;
        }
        root.selectedIndex =
            (root.selectedIndex + delta + root.cellCount) % root.cellCount;
    }

    function activateSelected() {
        if (root.selectedIndex < 0) return false;
        const ws = root.workspaceGroup * root.workspacesShown + root.selectedIndex + 1;
        Hyprland.dispatch(`hl.dsp.focus({ workspace = ${ws} })`);
        return true;
    }

    Component.onCompleted: {
        root.selectedIndex = root.effectiveActiveWorkspaceId
            - (root.workspaceGroup * root.workspacesShown) - 1;
    }
    ...
    property real workspaceSpacing: 10 // estilo igual a la vista de ventanas
    ...
    Rectangle { // Background
                ...
                delegate: Row {
                    ...
                    Repeater { // Workspace repeater
                        ...
                        Rectangle { // Workspace
                            ...
                            border.width: (workspace.workspaceValue ===
                                root.workspaceGroup * root.workspacesShown + root.selectedIndex + 1) ? 3 : 1
                            border.color: (workspace.workspaceValue ===
                                root.workspaceGroup * root.workspacesShown + root.selectedIndex + 1)
                                ? Appearance.colors.colPrimary
                                : hoveredWhileDragging
                                    ? hoveredBorderColor
                                    : ColorUtils.transparentize(
                                        Appearance.colors.colOnLayer1, 0.88)
        ...
        Item { // Windows & focused workspace indicator
            ...
            Rectangle { // Focused workspace indicator
                ...
                // El borde de seleccion con Tab ya marca el workspace activo.
                visible: false
