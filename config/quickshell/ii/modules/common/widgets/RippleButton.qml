...
Button {
    ...
    property var ctrlAction // When left clicking with Ctrl held
    property bool ctrlHandled: false
    ...
    MouseArea {
        ...
        onPressed: (event) => { 
            ...
            if ((event.modifiers & Qt.ControlModifier) && root.ctrlAction) {
                root.ctrlHandled = true;
                root.ctrlAction(event);
                return;
            }
            root.ctrlHandled = false;
            root.down = true
