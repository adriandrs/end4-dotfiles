...
DockButton {
    ...
    Loader {
        ...
        sourceComponent: MouseArea {
            ...
            // Rueda: cicla por las ventanas de esta app y las trae al frente.
            onWheel: wheel => {
                const list = appToplevel.toplevels;
                if (list.length === 0) return;

                const step = wheel.angleDelta.y > 0 ? -1 : 1;
                lastFocused = ((lastFocused + step) % list.length + list.length)
                    % list.length;

                const raw = list[lastFocused]?.HyprlandToplevel?.address;
    ...
    onClicked: {
        ...
        // Solo ventanas del workspace actual (el modulo cubre las minimizadas).
        ...
        const pool = candidatesHere.length > 0
            ? candidatesHere
        ...
        const workspaceId =
            HyprlandData.windowByAddress[address]?.workspace?.id ?? 0;
    ...
    // Boton central: cierra una ventana de esta app, la mas reciente.
    middleClickAction: () => {
        const list = appToplevel.toplevels;
    ...
    // Clic derecho: abre otra instancia de la app.
    altAction: () => {
        root.desktopEntry?.execute();
    }
