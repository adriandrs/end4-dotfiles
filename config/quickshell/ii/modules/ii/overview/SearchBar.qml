...
RowLayout {
    ...
    signal overviewNavRequested(int delta)
    signal overviewActivateRequested()
    ...
    ToolbarTextField { // Search box
        ...
        Keys.onPressed: event => {
            // Sin texto de busqueda, Tab/Enter navegan las miniaturas.
            if (root.searchingText === "") {
                if (event.key === Qt.Key_Tab) {
                    root.overviewNavRequested(1);
                    event.accepted = true;
                    return;
                }
                ...
                if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    root.overviewActivateRequested();
                    event.accepted = true;
                    return;
                }
            }
