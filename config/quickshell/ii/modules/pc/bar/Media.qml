...
Item {
    ...
    property bool borderless: PcConfig.options.bar.borderless
    ...
    implicitWidth: vertical 
        ...
        : (isMaterial 
            ...
            : Math.max(
                PcConfig.options.bar.media.minWidth,
                Math.min(rowLayout.implicitWidth + 8, PcConfig.options.bar.media.maxWidth)
            ))
    implicitHeight: vertical ? (isMaterial ? 32 : mediaCircProg.implicitHeight) : Appearance.sizes.barHeight

    Timer {
        running: activePlayer?.playbackState == MprisPlaybackState.Playing
        interval: PcConfig.options.resources.updateInterval
        repeat: true
    ...
    Loader {
        ...
        sourceComponent: RowLayout {
            ...
            StyledText {
                visible: PcConfig.options.bar.verbose
                Layout.alignment: Qt.AlignVCenter
                ...
                text: PcConfig.options.bar.media.onlyTitle ? root.cleanedTitle : `${root.cleanedTitle}${root.activePlayer?.trackArtist ? ' • ' + root.activePlayer.trackArtist : ''}`
            }
        }
    }
            ...
            Loader {
                ...
                sourceComponent: RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        StyledText {
                            text: PcConfig.options.profile.displayName === "" ? SystemInfo.username : PcConfig.options.profile.displayName
