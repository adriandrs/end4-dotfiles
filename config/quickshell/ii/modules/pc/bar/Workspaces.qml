...
ButtonMouseArea {
    ...
    property real workspaceButtonWidth: PcConfig.options.bar.cornerStyle === 3 ? 30 : 26
        ...
        WorkspaceLayout {
            ...
            Repeater {
                ...
                delegate: WorkspaceItem {
                    ...
                    AppIcon {
                        id: appIcon
                        property real cornerMargin: (!root.superPressAndHeld && PcConfig.options?.bar.workspaces.showAppIcons && wsApp.biggestWindow) ? (root.workspaceButtonWidth - root.workspaceIconSize) / 2 : root.workspaceIconMarginShrinked
                        anchors {
                    ...
                    Loader { // Somehow putting this multieffect in a loader prevents it from not showing up
                        ...
                        sourceComponent: Colorizer {
                            ...
                            colorization: PcConfig.options.bar.workspaces.monochromeIcons ? 0.8 : 0.5
                            brightness: 0
                            source: appIcon

                            opacity: !PcConfig.options?.bar.workspaces.showAppIcons ? 0 : (wsApp.biggestWindow && !root.superPressAndHeld && PcConfig.options?.bar.workspaces.showAppIcons) ? 1 : wsApp.biggestWindow ? root.workspaceIconOpacityShrinked : 0
                            visible: opacity > 0
                            scale: ((!root.superPressAndHeld && PcConfig.options?.bar.workspaces.showAppIcons) ? root.workspaceIconSize : root.workspaceIconSizeShrinked) / root.workspaceIconSize
    ...
    component NumberWorkspaceItem: WorkspaceItem {
        ...
        property bool showingNumbers: {
            ...
            if (GlobalStates.screenLocked)
                return false;
            if (PcConfig.options?.bar.workspaces.alwaysShowNumbers && (!PcConfig.options?.bar.workspaces.showAppIcons || !wsNum.hasBiggestWindow))
                return true;
            return false;
        }
        ...
        FadeLoader {
            ...
            Loader {
                anchors.centerIn: parent
                sourceComponent: (PcConfig.options?.bar.workspaces.indicatorStyle ?? "dot") === "icon" ? iconComponent : dotComponent
        ...
        FadeLoader {
            ...
            StyledText {
                ...
                font {
                    ...
                    family: PcConfig.options?.bar.workspaces.useNerdFont ? Appearance.font.family.iconNerd : defaultFont
                }
                color: wsNum.contentColor
                text: PcConfig.options?.bar.workspaces.numberMap[wsNum.wsId - 1] || wsNum.wsId
            }
        }
    }
