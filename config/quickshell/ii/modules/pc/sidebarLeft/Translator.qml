...
import qs.modules.pc.sidebarLeft.translator
import QtQuick
...
Item {
    ...
    // Options
    property string targetLanguage: PcConfig.options.language.translator.targetLanguage
    property string sourceLanguage: PcConfig.options.language.translator.sourceLanguage
    ...
    function swapLanguages() {
        ...
        PcConfig.options.language.translator.targetLanguage = root.targetLanguage;
        PcConfig.options.language.translator.sourceLanguage = root.sourceLanguage;
    ...
    Timer {
        id: translateTimer
        interval: PcConfig.options.sidebar.translator.delay
        repeat: false
        onTriggered: () => {
    ...
    ColumnLayout {
        ...
        TextCanvas {
            ...
            GroupButton {
                ...
                onClicked: {
                    let url = PcConfig.options.search.engineBaseUrl + outputCanvas.displayedText;
                    for (let site of PcConfig.options.search.excludedSites) {
                        url += ` -site:${site}`;
                    }
        ...
        sourceComponent: SelectionDialog {
            ...
            onSelected: (result) => {
                ...
                if (root.languageSelectorTarget) {
                    ...
                    PcConfig.options.language.translator.targetLanguage = result;
                } else {
                    root.sourceLanguage = result;
                    PcConfig.options.language.translator.sourceLanguage = result;
                }
                translateTimer.restart();
            }
        }
