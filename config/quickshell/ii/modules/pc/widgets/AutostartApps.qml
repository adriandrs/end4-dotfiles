...
ColumnLayout {
    ...
    function addEntry() {
        let list = []
        for (let i = 0; i < PcConfig.options.hyprland.autostartApps.apps.length; i++) {
            let o = PcConfig.options.hyprland.autostartApps.apps[i]
            list.push({ cmd: o.cmd, workspace: o.workspace, delay: o.delay })
        }
        list.push({ cmd: "", workspace: 1, delay: 0 })
        PcConfig.options.hyprland.autostartApps.apps = list
    }
    ...
    RowLayout {
        ...
        GroupedList {
            ...
            ConfigSwitch {
                ...
                checked: PcConfig.options.hyprland.autostartApps.enable
                onCheckedChanged: {
                    PcConfig.options.hyprland.autostartApps.enable = checked
                }
            }
        }
        ...
        RippleButton {
            ...
            visible: PcConfig.options.hyprland.autostartApps.enable
    ...
    Item {
        ...
        height: 24
        visible: PcConfig.options.hyprland.autostartApps.apps.length > 0 && PcConfig.options.hyprland.autostartApps.enable
    ...
    Repeater {
        id: appsRepeater
        model: PcConfig.options.hyprland.autostartApps.apps
