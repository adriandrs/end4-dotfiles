...
Item {
    ...
    property var pinnedApps: PcConfig.options?.dock.pinnedApps ?? []
    ...
    function commitOrder() {
        ...
        PcConfig.options.dock.pinnedApps = newOrder
        ...
        delegate: Item {
            ...
            DockButton {
                ...
                onClicked: {
                    ...
                    console.log("[dock] tls=" + (entry?.toplevels?.length ?? -1) + " desk=" + !!slotItem.deskEntry + " appId=" + slotItem.appId);
                    if (!entry || entry.toplevels.length === 0) {
                        slotItem.deskEntry?.execute()
                        return
                    }
                    const MINIMIZE_OFFSET = 1000;
                    const currentWs = HyprlandData.activeWorkspace?.id ?? 1;
                    const candidatesHere = entry.toplevels.filter(tl => {
                        const r = tl?.HyprlandToplevel?.address;
                        if (!r) return false;
                        const d = HyprlandData.windowByAddress[`0x${r}`];
                        const id = d?.workspace?.id ?? 0;
                        return id > 0 && (id % 1000) === (currentWs % 1000);
                    });
                    const pool = candidatesHere.length > 0 ? candidatesHere : entry.toplevels;
                    const next = (slotItem._lastFocused + 1) % pool.length;
                    slotItem._lastFocused = next;
                    const target = pool[next];
                    const raw = target?.HyprlandToplevel?.address;
                    console.log("[dock] raw=" + raw);
                    const address = raw ? `0x${raw}` : "";
                    if (address === "") {
                        target.activate();
                        return;
                    }
                    const workspaceId = HyprlandData.windowByAddress[address]?.workspace?.id ?? 0;
                    if (workspaceId > MINIMIZE_OFFSET) {
                        const orig = workspaceId % MINIMIZE_OFFSET;
                        Hyprland.dispatch(`hl.dsp.window.move({workspace = "${orig}", window = "address:${address}", follow = false})`);
                        ...
                        Hyprland.dispatch(`hl.dsp.window.move({workspace = "${workspaceId + MINIMIZE_OFFSET}", window = "address:${address}", follow = false})`);
                    } else {
                        Hyprland.dispatch(`hl.dsp.focus({window = "address:${address}"})`);
                        Hyprland.dispatch(`hl.dsp.window.alter_zorder({mode = "top", window = "address:${address}"})`);
                    }
                ...
                WheelHandler {
                    ...
                    onWheel: event => {
                        const tls = slotItem.appEntry?.toplevels ?? [];
                        if (tls.length === 0) return;
                        const dir = event.angleDelta.y > 0 ? -1 : 1;
                        const next = (slotItem._lastFocused + dir + tls.length) % tls.length;
                        slotItem._lastFocused = next;
                        tls[next]?.activate();
                    }
                }
                altAction:         () => {
                    const tls = slotItem.appEntry?.toplevels ?? [];
                    if (tls.length > 0) tls[0]?.close();
                    else TaskbarApps.togglePin(slotItem.appId);
                }
                ...
                contentItem: Item {
                    ...
                    Loader {
                        active: PcConfig.options.dock.monochromeIcons
    ...
    PopupWindow {
        ...
        property bool shouldShow: false && WM.compositor === "hyprland"
