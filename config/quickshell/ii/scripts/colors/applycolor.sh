...
apply_kitty() {  
  ...
  theme_file="$STATE_DIR/user/generated/terminal/kitty-theme.conf"

  for pid in $(pgrep -x kitty); do
    timeout 2 kitten @ --to "unix:@kitty-end4-$pid" set-colors \
      --all \
      --configured \
...
apply_term() {
  # apply_anyterm escribe secuencias de escape en /dev/pts/*, lo que
  # congela la terminal activa. kitty ya recibe colores por socket.
  # apply_anyterm &
  apply_kitty &
}
